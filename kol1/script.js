...
- option: VDI tehnologija generiše prednosti kada se izvršava aplikacija koja zahteva 3D renderovanje
...
Q: Da li je sledeća tvrdnja tačna? Konvergirana infrastruktura može da se definiše kao IT infrastruktura koja ne koristi najbolje prakse iz računarstva u procesu izbora serverskih, mrežnih i resursa za skladištenje podataka, koji se alociraju (eng. "allocate") na zahtev (eng. "on-demand") raznim operativnim okruženjima i aplikacijama.
- answer: Ne
- option: Da
...
Q: Standard TIA/EIA-568 odnosi se na:
- option: Žične mreže
- option: HP servere
...
Q: Da li je sledeća tvrdnja tačna? Virtuelizacija servera je primarna komponenta koja se koristi u izradnji hiper-konvergiranog okruženja, koje se najčešće koristi u klaud računarstvu. U ovako postavljenom okruženju, virtuelne mašine se izvršavaju na računaru domaćinu (eng. "Host computer") i takvo okruženje omogućava da više servera koegzistira na jednoj hardverskoj platformi.
- option: Ne
- answer: Da
...
- option: Industrijski standard za transfer fajlova putem šifrovanog kanala.
- option: Siguran protocol koji služi za deljenje fajlova između Windows računara.
- option: Industrijski standard za transfer fajlova u formi otvorenog (eng. "plain") teksta.
...
Q: Koji od sledećih iskaza je tačan:
- answer: Hub and spoke topologija je slična topologiji zvezde
- option: Potpuno zamršena topologija je slična topologiji zvezve
- option: Klaud DHCP protocol u pojedinim slučajevima koristi DORA model
- option: Konvegirane tehnologije se najčešće koriste u implementaciji mrežnih komutatora trećeg sloja
- option: Ništa od navedenog nije tačno
- option: Delimično zamržena topologija se najčešće koristi kod lokalnih mreža

Q: Prema Gartner-u neke od pet glavnih karakteristika klaud računarstva su (označiti 3 uljeza):
- option: merenje po potrošnji
- answer: korišćenje HTTP protokola
- answer: korišćenje internet tehnologija
- option: dostavljanje resursa na zahtev
- option: skalabilnost
- answer: orijentisanost ka virtuelnim mašinama

Q: Koja od sledećih tvrdnji koja se odnosi na tehnologiju virtualizacije je tačna:
- option: Operativni system "vidi" grupu fizičkih server kao jedan skup računarskih resursa i više operativnih sistema može paralelno da se izvršava na jednoj fizičkoj mašini (serveru).
- option: Bez višenitnog izvršavanja tehnologije virtualizacije ne bi bilo moguće
- option: Ništa od nevedenog nije tačno
- answer: Virtualizacija podrazumeva enkapsulaciju I apstrakciju računarskih resursa, kada se oni koriste na način koji odgovara određenoj primeni
- option: Jedan od oblasti koju virtuelizacija obuhvata je virtuelizacija izlaznih uređaja
...
- option: U širokopojasnim mrežama najčešće se koristi topologija zvezva i linkovi
- answer: proxy server koji optimizuju pristup internet tako što keširaju (eng. caching) sadržaj i vrše autorizovanje Web saobraćaja
- option: Tačan je samo jedan od ponuđenih
- option: WINS (Windows Internet Naming Service) server koji razrešavaju NetBIOS (Network Basic Input Output System) imena u IP adrese
- answer: Računski centar treba da obezbedi kontrolu okruženja da bi se toplota koju generiše oprema na odgovarajući način rasipala I da bi se oprema hladila do optimalne radne temperature
- option: CIFS je takoše poznat pod nazivom SMBP

Q: Glavna snaga virtualizacije leži u:
- answer: Particionisanju
- option: Departicionisanju
- option: Dekonponiciji
- option: Segregaciji
- option: Agregaciji
- option: Kompoziciji

Q: Neki od modova (opcija) za konfigurisanje timova mrežnih kartica su (označi 2 tačna odgovora):
- option: Transmit Load Balancing with Failover Protection and Preference Sequence
- answer: NFT
- answer: SLB
- option: Network Fault Tolerance Only with Sequence Order
...
- answer: kada je potrebno da se upravlja životnim ciklusom više aplikacija
- option: kada se razvija i testira jedna izolovana aplikacija
- option: kada se razvija troslojna aplikacija Web aplikacija
- answer: kada postoje zahtevi u pogledu raspoloživosti računarskih resursa različitim poslovnim jedinicama koje su međusobno izolovane

Q: U navedenoj listi označiti prednosti IaaS modela (označiti 3 tačna odgovora):
- option: izbegavanje troškova koji bi nastali kupovinom licenci za korišćenje aplikacija
- option: za razliku od SaaS I PaaS modela, klijent je odgovoran za sve aspekte upravljanja virtuelnim mašinama
- answer: focus na rast i razvoj poslovanja
- answer: laka administracija virtuelnih mašina
- answer: skalabilnost i fleksibilnost
- option: zavisnost od mreže klaud prvajdera

Q: Standard 802.11ac podržava frekvenciju:
- option: 2.4MHz
- option: 5MHz i 2.4MHz
- answer: 5MHz
...
- answer: Virtuelizacija procesora
- option: Virtuelizacija mreže
- option: Virtuelizacija server
- answer: Virtuelizacija diska

Q: Koje su od sledećih tvrdnji tačne (označi 2 tačna odgovora):
- option: Infrastruktura javnog klauda je u vlasništvu poslovne, vladine ili akademske organizacije koje su potpuno odgovorne za njegovu sigurnost i bezbednost.
- option: Jedan od najvećih nedostataka privatnog klauda je nepostojanje striktnih protokola za upravljanje podacima
- answer: Ako se posmatra iz perspective privatnih (ličnih) korisnika klaud usluga, javni klaud predstavlja primenu koncepta spoljneg snabdevanja
- answer: Modeli javnog klauda najviše se koriste za hostovanje razvojnih platform za obradu velike količine podataka i za kompanije koje nemaju visoke standard po pitanju sigurnosti i bezbednosti podataka i sistema.
- option: Jedna od najvećih prednosti privatnog klauda je troškovna efikasnost.

Q: Neke od prednosti koje omogućava primena SaaS su sledeće (označiti 3 uljeza):
- answer: slaba kontrola nad razvojem, nadogradnjom i testiranjem aplikacija
- option: potpuna kontrola nad virtuelnim mašinama
- option: izbegavanje troškova koji bi nastali kupovinom licenci za korišćenje aplikacija
- option: brz razvoj, jer se softwer obezbeđuje na zahtev
- answer: klijent nema kontrolu nad sistemom koji obrađuje i skladišti njegove podatke
- option: optimalna upotreba raspoloživih računarskih resursa
- option: eliminacija vremena, utrošene energije i troškova koji bi nastali na osnovu implementacije i održavanja svih ovih servisa
